...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, SelectButtonModule, DrawerModule, RadioButtonModule, ButtonModule, SkeletonModule],
...
export class Configurator implements OnInit {
    ...
    layoutTheme = computed(() => this.layoutService.layoutConfig().layoutTheme);

    // primaryColors están comentados temporalmente — se reactivarán cuando el backend esté listo
    ...
    set menuMode(val: string) {
        this.layoutService.changeMenuMode(val);
    }
    ...
    private loadConfigFromProfile(): void {
        ...
        const savedConfig = profile?.metadata?.['uiConfig'] as Partial<LayoutConfig> | undefined;

        if (savedConfig) {
            this.layoutService.layoutConfig.set({
                ...DEFAULT_CONFIG,
                ...savedConfig,
                // Preservamos primary hardcodeado hasta que el backend soporte primary colors
    ...
    async saveConfig(): Promise<void> {
        ...
        const uiConfig: Partial<LayoutConfig> = {
            preset: currentConfig.preset,
            surface: currentConfig.surface,
            darkTheme: currentConfig.darkTheme,
            menuMode: currentConfig.menuMode,
            layoutTheme: currentConfig.layoutTheme
            // primary se omite hasta que el backend soporte primary colors
        };
        ...
        try {
            const updated = await firstValueFrom(
                this.profileService.updateProfile(profile.id, {
                    metadata: { ...profile.metadata, uiConfig }
                })
            );
    ...
    getPresetExt() {
        // Usa el primary color del config para construir la extensión del preset
        const presetPalette = presets[this.layoutService.layoutConfig().preset as KeyOfType<typeof presets>].primitive;
        const colors = ['emerald', 'green', 'lime', 'orange', 'amber', 'yellow', 'teal', 'cyan', 'sky', 'blue', 'indigo', 'violet', 'purple', 'fuchsia', 'pink', 'rose'];
        const palettes: SurfacesType[] = [{ name: 'noir', palette: {} }];
        ...
        const color: SurfacesType = palettes.find((c) => c.name === this.selectedPrimaryColor()) || {};
