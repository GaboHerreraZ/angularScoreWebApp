...
### Request esperado desde el frontend

El frontend envía el archivo como `multipart/form-data` con el campo `file`:

```
POST /api/companies/{companyId}/credit-studies/extract-pdf
Content-Type: multipart/form-data

file: <archivo.pdf>  (campo "file", máximo 10 MB, solo application/pdf)
```

El backend debe:
1. Recibir el archivo del campo `file` del FormData
2. Validar que sea PDF y no exceda 10 MB
3. Convertir el buffer a base64 y enviarlo a Claude API
4. Retornar el JSON extraído directamente como respuesta
